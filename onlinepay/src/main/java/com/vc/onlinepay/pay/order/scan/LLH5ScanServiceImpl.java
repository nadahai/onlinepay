...
@Service
@Component
public class LLH5ScanServiceImpl {

    private static Logger logger = LoggerFactory.getLogger (LLH5ScanServiceImpl.class);
    ...
    public JSONObject payOrder (JSONObject reqData, ResultListener listener) {
        ...
        try {
            logger.info ("乐乐支付宝H5接收入参{}", reqData);
            String ddh = reqData.getString ("vcOrderNo");
            result.put ("orderNo", ddh);
            String url = StringUtils.deleteWhitespace (reqData.getString ("channelPayUrl"));
            String shid = StringUtils.deleteWhitespace (reqData.getString ("channelKey"));
            String key = StringUtils.deleteWhitespace (reqData.getString ("channelDesKey"));
            String ybtz = reqData.getString ("projectDomainUrl") + "/lLH5CallBackController";
            String tbtz = reqData.getString("projectDomainUrl")+"/success";
            ...
            String je = df.format(new BigDecimal(amount));
            String zftd = "alipay";
            String ddmc = ddh;
            String ddbz = ddh;
            
            String signStr="shid="+shid+"&bb="+bb+"&zftd="+zftd
            		+"&ddh="+ddh+"&je="+je+"&ddmc="+ddmc+"&ddbz="+ddbz
            		+"&ybtz="+ybtz+"&tbtz="+tbtz+"&"+key;
            System.out.println(signStr);
            String sign = DigestUtils.md5DigestAsHex(signStr.getBytes());
            
            String payParm = "url="+url+"&bb="+bb+"&shid="+shid
            		+"&ddh="+ddh
            		+"&je="+je
            		+"&zftd="+zftd
            		+"&ybtz="+ybtz
    ...
    public static void main (String[] args) {
        try {
            ...
            String url = "http://pay.huif9518.com/pay/api.php";
            String shid = "10054";
            String key = "by7jz4MTOO4ArkO5tDCaVqIfHC7ybxOYtTsHWi5b";
            String bb="1.0";
            String ddh = (System.currentTimeMillis () + "").substring (0,13);
            String je = "200.00";
            String zftd = "alipay";
            String ybtz = "http://www.baidu.com";
            String tbtz = "http://pay.mastepay.com/Pay/yunPay/TUNPay.php";
            String ddmc = ddh;
            String ddbz = ddh;
            ...
            String payUrl = url+"?bb="+bb+"&shid="+shid
            		+"&ddh="+ddh
            		+"&je="+je
            		+"&zftd="+zftd
            		+"&ybtz="+ybtz
