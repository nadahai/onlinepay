...
public class H5ServiceApi {
    ...
    @Autowired
    private HongYuPayScanServiceImpl hongYuPayScanService;
    @Autowired
    private AnYiPayScanServiceImpl anYiPayScanService;
    @Autowired
    private ShanFuTongPayScanServiceImpl shanFuTongPayScanService;
    @Autowired
    private ZhongLianServiceImpl zhongLianService;
    @Autowired
    private QuBaoPayServiceImpl quBaoPayService;
    @Autowired
    private HeZongPayServiceImpl heZongPayService;
    ...
    public JSONObject doRestPay (JSONObject reqData) {
        try {
            ...
            switch (source) {
                ...
                case 305:
                case 306:
                    return hongYuPayScanService.payOrder(reqData, tradeCmd.tradResultListener(reqData));
                case 307:
                    return anYiPayScanService.payOrder(reqData, tradeCmd.tradResultListener(reqData));
                case 308:
                    return shanFuTongPayScanService.payOrder(reqData, tradeCmd.tradResultListener(reqData));
                case 309:
                    return zhongLianService.payOrder(reqData, tradeCmd.tradResultListener(reqData));
                case 310:
                    return quBaoPayService.payOrder(reqData, tradeCmd.tradResultListener(reqData));
                case 311:
                    return heZongPayService.payOrder(reqData, tradeCmd.tradResultListener(reqData));
                default:
