 ...
 */
@RestController
@RequestMapping("/pingAn2CallBackController")
public class PingAn2CallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
        try {
            ...
            logger.info("平安支付2WX回调接口接收参数:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("平安支付2WX回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("平安支付2WX回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            ...
            if("1".equals(requestJson.getString("fxstatus"))) {
            	...
            	logger.error("平安支付2WX回调接口支付失败{}", requestJson);
           	 	status = 3;
            }
            ...
            if (isOK) {
                ...
                logger.error("平安支付2WX回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("平安支付2WX回调接口业务处理异常", e);
