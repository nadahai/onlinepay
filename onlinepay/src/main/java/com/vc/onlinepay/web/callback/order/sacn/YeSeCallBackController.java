 ...
 */
@RestController
@RequestMapping("/yeSeCallBackController")
public class YeSeCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("夜色支付回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("夜色支付回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("夜色支付回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            ...
            if (isOK) {
                ...
                logger.error("夜色支付回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("夜色支付回调接口业务处理异常", e);
