 ...
 */
@RestController
@RequestMapping("/erYuanZHCallBackController")
public class ErYuanZHCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("二元智慧支付回调接口接收参数2:{}", request.toString());
            logger.info("二元智慧支付回调接口接收参数getParameter:{}", request.getParameter("orderid"));
            //logger.info("二元智慧支付回调接口接收参数getAttribute:{}", request.getAttribute("orderid"));
            ...
            String sourctxt1 = Md5CoreUtil.getSignStr(parms)+"&key=m1jb9wtg9cyujcdosf8valtaqv6bzdcw";
            ...
            if (isOK) {
                ...
                logger.error("二元智慧支付回调接口更新失败{}", request.toString());
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
