 ...
 */
@RestController
@RequestMapping("/wxSXBCallBackController")
public class WxSXBCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("微信扫码随心宝回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("微信扫码随心宝回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("微信扫码随心宝回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            
            int status =0;
            if ("1".equals(requestJson.getString("status"))) {
                status = 4;

            }else {
                status = 3;
            }
            boolean isOK = commonCallBackServiceImpl.callBackOrder2(vcOnlineOrder, status,requestJson.toString(),null);
            if (isOK) {
                return "success";
            } else {
                logger.error("微信扫码随心宝回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("微信扫码随心宝回调接口业务处理异常", e);
