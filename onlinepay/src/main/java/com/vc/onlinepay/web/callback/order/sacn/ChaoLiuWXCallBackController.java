 ...
 */
@RestController
@RequestMapping("/chaoLiuWXCallBackController")
public class ChaoLiuWXCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("潮流微信回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("潮流微信回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("潮流微信回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            ...
            String sign = Md5Util.md5(orderid  +platform_trade_no +price +realprice + "7082BE7253EFC716").toUpperCase();
            int status =0;
            ...
            if (isOK) {
                ...
                logger.error("潮流微信回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("潮流微信回调接口业务处理异常", e);
