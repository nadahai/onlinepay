...
@Controller
@RequestMapping("/hongYuPayCallBackApi")
public class HongYuPayScanCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            logger.info("鸿宇支付回调接口接收参数:{}", requestMap);

            if (requestMap == null || requestMap.isEmpty()){
                logger.error("鸿宇支付回调接口获取参数为空");
                return "failed";
            }
            String vcOrderNo = requestMap.getString("orderid");
            if (StringUtil.isEmpty(vcOrderNo)) {
                logger.error("鸿宇支付回调接口订单号为空{}", vcOrderNo);
                return "failed";
            }
            ...
            if(Math.abs(traAmount-callBackAmount)>2){
            	logger.error("鸿宇支付回调接口金额浮动超过2元:{}", vcOrderNo);
                return "failed";
            }
            ...
            if (isOk) {
                ...
                logger.error("鸿宇支付回调接口更新失败{}", requestMap);
                return "failed";
            }
        } catch (Exception e) {
            logger.error("鸿宇支付回调接口业务处理异常", e);
            return "failed";
        }
    }
    ...
    private boolean checkSign(String orderNo, JSONObject reqData, String md5Key) {
        try {
            ...
            signData.put("memberid",reqData.getString("memberid"));
            signData.put("orderid",reqData.getString("orderid"));
            signData.put("amount",reqData.getString("amount"));
            signData.put("transaction_id",reqData.getString("transaction_id"));
            signData.put("datetime",reqData.getString("datetime"));
            signData.put("returncode",reqData.getString("returncode"));
            String sign = Md5CoreUtil.md5ascii(signData, md5Key);
            if (sign.equalsIgnoreCase(pSign)) {
                return true;
            }
            logger.error("鸿宇支付订单回调验签失败:{},sign原串{}", orderNo, signData);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("鸿宇支付回调验签失败", orderNo, null, reqData.toString(), pSign));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "鸿宇支付回调验签失败!", orderNo));
        } catch (Exception e) {
            logger.error("鸿宇支付订单回调验签异常:{}", orderNo, e);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("鸿宇支付回调验签失败", orderNo, null, reqData.toString(), reqData.getString("sign")));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "鸿宇支付回调验签失败!", orderNo));
            return false;
        }
        return false;
