...
public class TaoBDFCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("淘宝代付支付回调接口接收参数2:{}", request.toString());
            logger.info("淘宝代付支付回调接口接收参数getParameter:{}", request.getParameter("orderid"));
            //logger.info("淘宝代付支付回调接口接收参数getAttribute:{}", request.getAttribute("orderid"));
            if (request.getParameter("orderid") == null || request.getParameter("orderid").isEmpty()){
                logger.error("淘宝代付支付回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("淘宝代付支付回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            ...
            String sourctxt1 = Md5CoreUtil.getSignStr(parms)+"&key=r0gxi60ej68wnr5mx4xopcge82g522q8";
            ...
            if (isOK) {
                ...
                logger.error("淘宝代付支付回调接口更新失败{}", request.toString());
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("淘宝代付支付回调接口业务处理异常", e);
