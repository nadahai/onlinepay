 ...
 */
@RestController
@RequestMapping("/quickPayCallbackApi")
public class QuickPayCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
        try {
            ...
            logger.info("速付回调接口接收参数:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("速付回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("速付回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            //状态 1下单成功 2下单失败 3下单中 4支付成功 5支付失败
            String payStatus = requestJson.getString("returncode");
            int status =0;
            if ("00".equalsIgnoreCase(payStatus)) {
                status = 4;
            }else {
                status = 3;
            }
            ...
            if (isOK) {
                ...
                logger.error("速付回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("速付回调接口业务处理异常", e);
