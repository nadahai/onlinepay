...
@Controller
@RestController
@RequestMapping("/huaFeiNewCallBackController")
public class HuaFeiNewCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            logger.info("话费新通道回调接口接收参数:{}", requestMap);

            if (requestMap == null || requestMap.isEmpty()){
                logger.error("话费新通道回调接口获取参数为空");
                return "ERROR";
            }
            String vcOrderNo = requestMap.get("id");
            if (StringUtil.isEmpty(vcOrderNo)) {
                logger.error("话费新通道回调接口订单号为空{}", vcOrderNo);
                return "ERROR";
            }
            int status = 0 ;
            ...
            String payStatus = requestMap.get("state");
            ...
            if (isOk) {
                ...
                logger.error("话费新通道回调结果{}更新失败{}",isOk,requestMap);
                return "ERROR";
            }
        } catch (Exception e) {
            logger.error("话费新通道回调接口业务处理异常", e);
            return "ERROR";
        }
    }
}
