...
@Controller
@RequestMapping("/quBaoPayCallBackApi")
public class QuBaoPayCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            logger.info("趣宝支付回调接口接收参数:{}", requestMap);

            if (requestMap == null || requestMap.isEmpty()){
                logger.error("趣宝支付回调接口获取参数为空");
                return "failed";
            }
            String vcOrderNo = requestMap.getString("out_trade_no");
            if (StringUtil.isEmpty(vcOrderNo)) {
                logger.error("趣宝支付回调接口订单号为空{}", vcOrderNo);
                return "failed";
            }
            ...
            Double callBackAmount = Double.valueOf(requestMap.getString("order_amount"));//回调返回交易金额
            if(Math.abs(traAmount-callBackAmount)>2){
            	logger.error("趣宝支付回调接口金额浮动超过2元:{}", vcOrderNo);
                return "failed";
            }
            ...
            if (isOk) {
                ...
                logger.error("趣宝支付回调接口更新失败{}", requestMap);
                return "failed";
            }
        } catch (Exception e) {
            logger.error("趣宝支付回调接口业务处理异常", e);
            return "failed";
        }
    }
    ...
    private boolean checkSign(String orderNo, JSONObject reqData, String md5Key) {
        try {
            ...
            //验签参数
            reqData.remove("sign");
            String sign = Md5CoreUtil.getSignStr(reqData);
            sign = Md5Util.MD5(sign+md5Key);
            if (sign.equalsIgnoreCase(pSign)) {
                return true;
            }
            logger.error("趣宝支付订单回调验签失败:{},sign原串{}", orderNo, reqData);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("趣宝支付回调验签失败", orderNo, null, reqData.toString(), pSign));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "趣宝支付回调验签失败!", orderNo));
        } catch (Exception e) {
            logger.error("趣宝支付订单回调验签异常:{}", orderNo, e);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("趣宝支付回调验签失败", orderNo, null, reqData.toString(), reqData.getString("sign")));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "趣宝支付回调验签失败!", orderNo));
            return false;
        }
        return false;
