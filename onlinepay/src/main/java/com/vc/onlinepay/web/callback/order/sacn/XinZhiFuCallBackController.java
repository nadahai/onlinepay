 ...
 */
@RestController
@RequestMapping("/xinZhiFuCallBackController")
public class XinZhiFuCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("鑫支付回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("鑫支付回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("鑫支付回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            
            int status =0;
            if ("SUCCESS".equals(requestJson.getString("tradeStatus"))) {
                status = 4;
            }else {
                status = 3;
            }
            ...
            if (isOK) {
                ...
                logger.error("鑫支付回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("鑫支付回调接口业务处理异常", e);
