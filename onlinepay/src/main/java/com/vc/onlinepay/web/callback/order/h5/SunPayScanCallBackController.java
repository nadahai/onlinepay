...
public class SunPayScanCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            logger.info("太阳支付回调接口接收参数:{}", requestMap);

            if (requestMap == null || requestMap.isEmpty()){
                logger.error("太阳支付回调接口获取参数为空");
                return "failed";
            }
            String vcOrderNo = requestMap.getString("mchOrderNo");
            if (StringUtil.isEmpty(vcOrderNo)) {
                logger.error("太阳支付回调接口订单号为空{}", vcOrderNo);
                return "failed";
            }
            ...
            if (isOk) {
                ...
                logger.error("太阳支付回调接口更新失败{}", requestMap);
                return "failed";
            }
        } catch (Exception e) {
            logger.error("太阳支付回调接口业务处理异常", e);
            return "failed";
        }
    }
    ...
    private boolean checkSign(String orderNo, JSONObject reqData, String md5Key) {
        try {
            ...
            logger.error("太阳支付订单回调验签失败:{},sign原串{}", orderNo, reqData);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("太阳支付回调验签失败", orderNo, null, reqData.toString(), pSign));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "太阳支付回调验签失败!", orderNo));
        } catch (Exception e) {
            logger.error("太阳支付订单回调验签异常:{}", orderNo, e);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("太阳支付回调验签失败", orderNo, null, reqData.toString(), reqData.getString("sign")));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "太阳支付回调验签失败!", orderNo));
            return false;
        }
        return false;
