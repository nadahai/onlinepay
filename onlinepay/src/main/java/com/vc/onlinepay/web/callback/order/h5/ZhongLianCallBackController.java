...
@Controller
@RequestMapping("/zhongLianPayCallBackApi")
public class ZhongLianCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            logger.info("众联支付回调接口接收参数:{}", requestMap);

            if (requestMap == null || requestMap.isEmpty()){
                logger.error("众联支付回调接口获取参数为空");
                return "failed";
            }
            String vcOrderNo = requestMap.getString("merchantOrderNo");
            if (StringUtil.isEmpty(vcOrderNo)) {
                logger.error("众联支付回调接口订单号为空{}", vcOrderNo);
                return "failed";
            }
            ...
            if(Math.abs(traAmount-callBackAmount)>200){
            	logger.error("众联支付回调接口金额浮动超过2元:{}", vcOrderNo);
                return "failed";
            }
    ...
    private boolean checkSign(String orderNo, JSONObject reqData, String md5Key) {
        try {
            ...
            reqData.remove("sign");
            //验签参数
            String signStr = Md5CoreUtil.getSignStr(reqData)+"&appkey="+md5Key;
            String sign = Md5Util.MD5(signStr);
            if (sign.equalsIgnoreCase(pSign)) {
                return true;
            }
            logger.error("众联支付订单回调验签失败:{},sign原串{}", orderNo, reqData);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("众联支付回调验签失败", orderNo, null, reqData.toString(), pSign));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "众联支付回调验签失败!", orderNo));
        } catch (Exception e) {
            logger.error("众联支付订单回调验签异常:{}", orderNo, e);
            CompletableFuture.runAsync(() -> commonPayService.saveLog("众联支付回调验签失败", orderNo, null, reqData.toString(), reqData.getString("sign")));
            asynNotice.asyncMsgNotice(new MessageModel(MessageTypeEnum.WINDOW, "众联支付回调验签失败!", orderNo));
            return false;
        }
        return false;
