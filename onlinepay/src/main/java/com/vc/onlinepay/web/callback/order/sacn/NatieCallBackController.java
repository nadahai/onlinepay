package com.vc.onlinepay.web.callback.order.sacn;

import com.alibaba.fastjson.JSONObject;
import com.vc.onlinepay.persistent.common.CommonCallBackService;
import com.vc.onlinepay.persistent.entity.online.VcOnlineOrder;
import com.vc.onlinepay.utils.Constant;
import com.vc.onlinepay.utils.Md5CoreUtil;
import com.vc.onlinepay.utils.Md5Util;
import com.vc.onlinepay.utils.StringUtil;
import com.vc.onlinepay.utils.http.HttpRequestTools;
import com.vc.onlinepay.web.base.BaseController;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * @author nada
 * @描述:拿铁支付宝支付回调处理
 * @时间:2018/12/26 15:23
 */
@RestController
@RequestMapping("/natieCallBackController")
public class NatieCallBackController extends BaseController {
    
    @Autowired
    private CommonCallBackService commonCallBackServiceImpl;
    
    /**
     * @描述:拿铁支付宝支付回调处理
     * @时间:2018年12月26日 下午15:24:18
     */
    @Override
    @RequestMapping(value = "", produces = "text/html;charset=UTF-8")
    public void doPost(HttpServletRequest request, HttpServletResponse response)  {
        setHttpServletContent(request, response);
        try {
            response.setContentType("text/html");
            String result = this.invokeCallback(request);
            response.getWriter().write(result);
        } catch (Exception e) {
            logger.error("拿铁支付宝回调接口异常", e);
            super.writeErrorResponse(response);
        }
    }

    /**
     * @描述:拿铁支付宝回调接口
     * @时间:2018年12月26日 下午15:24:18
     */
    public String invokeCallback(HttpServletRequest request) {
    	try {
            JSONObject requestJson = HttpRequestTools.getRequestJson(request);
            logger.info("拿铁支付宝支付回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("拿铁支付宝支付回调接口获取参数为空");
                return Constant.res_FAILED;
            }
            String vcOrderNo = requestJson.getString("custOrderNo");
            VcOnlineOrder vcOnlineOrder = payBusService.getVcOrderByorderNo(vcOrderNo);
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("拿铁支付宝支付回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            
            
            
            String version = requestJson.getString("version");
            String orgNo = requestJson.getString("orgNo");
            String custId = requestJson.getString("custId");
            String custOrderNo = requestJson.getString("custOrderNo");
            String prdOrdNo = requestJson.getString("prdOrdNo");
            String ordAmt = requestJson.getString("ordAmt");
            String ordTime = requestJson.getString("ordTime");
            String payAmt = requestJson.getString("payAmt");
            String ordStatus = requestJson.getString("ordStatus");
            
            JSONObject parms = new JSONObject();
            parms.put ("version",version);
            parms.put ("orgNo",orgNo);
            parms.put ("custId",custId);
            parms.put ("custOrderNo", custOrderNo);
            parms.put ("prdOrdNo", prdOrdNo);
            parms.put ("ordAmt",ordAmt);
            parms.put ("ordTime",ordTime);
            parms.put ("payAmt",payAmt);
            parms.put ("ordStatus",ordStatus);
            
            String keys = "";
            if("19062800002052".equals(custId)) {
            	keys = "F6EF72223B2200D1A5B48184D44D0ABD";
            }else if ("19070400002055".equals(custId)) {
            	keys = "3F1D1C7791CBF9E09C5E51B574973903";
            }else keys = "0BFBB311BF0C6CBCD08CCB2FB5522EBF";
            
            String sourctxt1 = Md5CoreUtil.getSignStr(parms)+"&key="+keys;
            logger.info("排序后{}",sourctxt1);
            String newSign = Md5Util.md5(sourctxt1).toUpperCase();
            
            String sign = requestJson.getString("sign").toUpperCase();
            
            logger.info("newSign,sing{},{}",newSign,sign);
            
            int status =0;
            if ("01".equals(ordStatus) && newSign.equals(sign)) {
                status = 4;
            }else {
                status = 3;
            }
            boolean isOK = commonCallBackServiceImpl.callBackOrder(vcOnlineOrder, status,requestJson.toString());
            if (isOK) {
                return "ok";
            } else {
                logger.error("拿铁支付宝支付回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("拿铁支付宝支付回调接口业务处理异常", e);
            return Constant.RES_ERROR;
        }
    }
}
