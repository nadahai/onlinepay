...
@Controller
@RestController
@RequestMapping("/sandPayCallBackApi")
public class SandCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            if (isOk) {
                ...
                logger.error("杉德银联扫码回调结果{}更新失败{}",isOk,requestMap);
                return "ERROR";
            }
        } catch (Exception e) {
            logger.error("杉德银联扫码回调接口业务处理异常", e);
            return "ERROR";
        }
    }
}
