...
@Controller
@RestController
@RequestMapping("/gaoYangPayCallBackApi")
public class GaoYangCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            logger.info("高阳银联回调接口接收参数:{}", requestMap);

            if (requestMap == null || requestMap.isEmpty()){
                logger.error("高阳银联回调接口获取参数为空");
                return "ERROR";
            }
            String vcOrderNo = requestMap.get("cOrderNo");
            if (StringUtil.isEmpty(vcOrderNo)) {
                logger.error("高阳银联回调接口订单号为空{}", vcOrderNo);
                return "ERROR";
            }
            int status = 0 ;
            vcOnlineOrder = payBusService.getVcOrderByorderNo(vcOrderNo);
            if(null==vcOnlineOrder){
                logger.error("高阳回调接口订单号未找到{}", vcOrderNo);
                return "ERROR";
            }
            ...
            if (isOk) {
                ...
                logger.error("高阳银联扫码回调结果{}更新失败{}",isOk,requestMap);
                return "ERROR";
            }
        } catch (Exception e) {
            logger.error("高阳银联扫码回调接口业务处理异常", e);
            return "ERROR";
        }
    }
}
