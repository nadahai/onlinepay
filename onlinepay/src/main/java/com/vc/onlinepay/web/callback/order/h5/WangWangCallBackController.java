 ...
 */
@RestController
@RequestMapping("/wangWangCallBackController")
public class WangWangCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("旺旺支付H5回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("旺旺支付H5回调接口获取参数为空");
                return Constant.res_FAILED;
            }
            String vcOrderNo = requestJson.getString("TradeNum");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("旺旺支付H5回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            
            
            
            String RespCode = requestJson.getString("RespCode");
            String Version = requestJson.getString("Version");
            String ApiMethod = requestJson.getString("ApiMethod");
            String ResultCode = requestJson.getString("ResultCode");
            String MerID = requestJson.getString("MerID");
            String Status = requestJson.getString("Status");
            String TradeNum = requestJson.getString("TradeNum");
            ...
            String keys = "GrjgBDseZotuGW54xd8GesHdcu8RIaIc";
            ...
            int status =0;
            if ("01".equals(Status) && newSign.equals(sign)) {
                status = 4;
            }else {
                status = 3;
            }
            ...
            if (isOK) {
                ...
                logger.error("旺旺支付H5回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("旺旺支付H5回调接口业务处理异常", e);
