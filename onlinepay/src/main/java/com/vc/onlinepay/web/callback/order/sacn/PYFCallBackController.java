 ...
 */
@RestController
@RequestMapping("/pYFCallBackController")
public class PYFCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
    	try {
            ...
            logger.info("PYF微信扫码支付回调接口接收参数2:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("PYF微信扫码支付回调接口获取参数为空");
                return Constant.res_FAILED;
            }
            String vcOrderNo = requestJson.getString("orderNo");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("PYF微信扫码支付回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            ...
            String sourctxt1 = Md5CoreUtil.getSignStr(parms)+"&key=778ce555d2231d0c2d69ffa7db9cff75";
            ...
            int status =0;
            if ("0".equals(status1) && newSign.equals(sign)) {
                status = 4;
            }else {
                status = 3;
            }
            ...
            if (isOK) {
                ...
                logger.error("PYF微信扫码支付回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("PYF微信扫码支付回调接口业务处理异常", e);
