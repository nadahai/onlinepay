 ...
 */
@RestController
@RequestMapping("/pingAnYunPayCallBackApi")
public class PingAnYunCallBackController extends BaseController {
    ...
    public String invokeCallback(HttpServletRequest request) {
        try {
            ...
            logger.info("平安WX回调接口接收参数:{}", requestJson);

            if (requestJson == null || requestJson.isEmpty()){
                logger.error("平安WX回调接口获取参数为空");
            ...
            if (StringUtil.isEmpty(vcOrderNo) || vcOnlineOrder==null) {
                logger.error("平安云WX回调接口订单号为空{}", vcOrderNo);
                return Constant.res_FAILED;
            }
            
            int status =0;
            double price = requestJson.getDouble("price");
            double realprice = requestJson.getDouble("realprice");
            //状态 1下单成功 2下单失败 3下单中 4支付成功 5支付失败 6:处理中 7:代付中 8:代付失败 9交易退款
            if(price == realprice) {
            	status = 4;
            }else {
            	logger.error("平安云WX回调接口实际支付金额与发起金额不一致{}", price+"====="+realprice);
           	 	status = 3;
            }
            ...
            if (isOK) {
                ...
                logger.error("平安云WX回调接口更新失败{}", requestJson);
                return Constant.res_FAILED;
            }
        } catch (Exception e) {
            logger.error("平安云WX回调接口业务处理异常", e);
