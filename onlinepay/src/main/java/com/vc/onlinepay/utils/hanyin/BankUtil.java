008343911,6217007110008659421,622908466087131410,6227001436110026695,6228481208086961578,6212262403015104049,6212264402074568357,6228480379404676270,6215582902000612870,6228481208230387076,6212261505000039906,6222021602018503344,6214832163064178,6217994910114091681,6215581509004781238,6217001140014316431,6221881600026006715,6212262108000982644,6217991680003956095,6222021603010887321,6212840207510007032,6217000450011340586,6230202017393308,6212260405011931,6225887579740875,6217211913001263105,6221506020028817974,6215581103008495019";
        String[] cardNos = cards.split(",");
        for (String cardNo : cardNos) {
            String bankName = testHybank(cardNo);
        }
    }

    private  static  String testHybank(String cardNo){
        String bankName = com.vc.onlinepay.utils.ninepie.BankUtil.getNoCardName(cardNo);
        if (Strings.isEmpty(bankName)||bankName.equals("暂未查到此银行")) {
            logger.error("无效银行卡号{}{}",cardNo);
            return null;
        }
        String blackBankNames = "中国工商银行,工商银行,兴业银行,广发银行,中信银行,交通银行,华夏银行,北京银行,光大银行,平安银行,中原银行,上海浦东发展银行";
        if(StringUtil.isNotEmpty(blackBankNames) && blackBankNames.contains(bankName)){
            logger.error("黑名单"+bankName);
            return null;
        }
        String passBankNames = "民生银行,招商银行,中国银行,农业银行,建设银行,邮政储蓄银行,邮储银行";
        if(StringUtil.isNotEmpty(passBankNames) && StringUtil.isNotEmpty(bankName)){
            if(passBankNames.contains(bankName)){
                //logger.error("白名单验证：{}",bankName);
                return null;
            }else{
                logger.error("不包含白名单验证：{}",bankName);
                return null;
            }
        }
        logger.info("无法解析:{}",cardNo);
        return bankName;
    }
    
    //获取银行名称
    public static String getNoCardName(String cardNumber){
    ...
    /**
     * @desc 获取银行名称，如果没有返回银行卡号
     * @author nada
     * @create 2019/6/14 10:56
    */
    public static String getCardNameOrNo(String cardNumber){
        String name = getNameOfBank(cardNumber);
        if (name.indexOf("·")!=-1) {
            name=name.substring(0,name.indexOf("·"));
        }
        ...
        if("未查到此银行卡".equalsIgnoreCase(name)){
        	logger.info("未查到此银行卡,银行卡号{}",cardNumber);
            return cardNumber;
        }
        return name;
    }
    ...
    "623251", "628351",
    "623052","623090","623036","623668","623059"};
    ...
    private static final String[] bankName = {   
     ...
     "华夏银行·华夏万通卡", "民生银行·民生借记卡", "民生银行·民生银联借记卡－金卡", "民生银行·钻石卡", "民生银行·民生借记卡", "民生银行·民生借记卡", "民生银行·民生借记卡",  
     ...
     "招商银行·银联IC普卡", "招商银行·借记卡", "招商银行·银联金葵花IC卡", "招商银行·IC公务卡", "招商银行·招商银行信用卡", "招商银行信用卡中心·美国运通绿卡", "招商银行信用卡中心·美国运通金卡",   
     ...
     "玉溪市商业银行·红塔卡",
     "中国农业银行","湖南省农村信用社(借记卡)","福建省农村信用社联合社","建设银行","河南省农村信用社联合社"};  
