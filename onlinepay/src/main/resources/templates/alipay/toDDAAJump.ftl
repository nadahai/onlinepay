...
<div class="am-content">
    ...
    <div class="result">
        ...
        <div class="result-tips" hidden="hidden">1、如果支付宝在后台运行,请先关闭,否则不能支付！</div>
        <div class="result-tips" hidden="hidden">2、如果依然拉起失败,请在UC浏览器打开后重试</div>
        <div class="result-botton"><a class="J-h5pay am-button am-button-blue" href="javascript:void(0)" id="paybtn" onclick="gopay()">立即支付</a></div>
        <div class="result-botton" hidden="hidden"><a class="J-h5pay am-button am-button-blue" href="javascript:void(0)" id="ucbtn" onclick="goucweb()">在UC打开</a></div>
    </div>
</div>
<script type="text/javascript">
    ...
    var data = {"requestType":"SafePay","fromAppUrlScheme":"alipays","dataString":service};
    function gopay(){
        try {
            //唤起客户端快捷参数
    ...
    function countDown(){
        if(time>0){
            ...
            $('.result-title').eq(0).html('支付宝官方提醒:目前充值人数较多，排队中。。。(' + time+ '秒)');
            $('#paybtn').html('请耐心等待 ' + time+ '秒');
        } else {
            $('#paybtn').html('继续支付');
            try {
                //唤起客户端快捷参数
    ...
    if(tips == "true"){
        Zepto.dialog.show({
            message: '<div style="color:#ff0000;font:16px bold;">如果支付宝在后台运行，请先关闭！否则不能支付！</div>',
            buttons: '确定',
            complete: function () {
                Zepto.dialog.hide();
            }
...
<!-- FD:256:mobileclientgw/home/v5/h5PayRoute.vm:END -->
<script type="text/javascript">
                ...
                Zepto.dialog.show({
                    ...
                    complete: function (result) {
                        if(result.order === 1) { // 下载支付宝App
                            ...
                            setTimeout(function(){$('.J-loading').addClass('fn-hide');}, 300);
                            sendCommlog('1');
...
</script><!-- CMS:安全支付客户端网关/common/footer.vm结束:mobileclientgw/common/footer.vm -->
