...
public class SerializerUtils  {
     ...
     * @return
     */
    public static <T> byte[] serializeList(List<T> value,Class clazz) {
        if (value == null) {
        ...
        try {
            ...
            for(T obj : value){
                os.writeObject(serializer.serialize(obj));
            }
            os.writeObject(null);
            os.close();
            bos.close();
     ...
     * @return
     */
    public static <T> List<T> unserializeForList(byte[] in,Class claszz) {
        List<T> list = new ArrayList<T>();
        ByteArrayInputStream bis = null;
        ObjectInputStream is = null;
        FastJsonRedisSerializer serializer = new FastJsonRedisSerializer(claszz);
        try {
            if(in != null) {
                bis=new ByteArrayInputStream(in);
                is=new ObjectInputStream(bis);
                while (true) {
                    Object obj = serializer.deserialize((byte[])is.readObject());
                    if(obj == null){
                        break;
                    }else{
                        list.add((T)obj);
